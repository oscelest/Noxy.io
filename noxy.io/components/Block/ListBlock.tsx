...
export default class ListBlock extends Component<ListBlockProps, State> {
  ...
  private readonly renderElement = (element: ListBlockText | HierarchyArray<ListBlockText>, key: number = 0) => {
    ...
    const ref = this.state.focus?.id === element.id ? this.state.ref : undefined;
    
    return (
      <li key={key}>
        <EditText ref={ref} key={key} readonly={this.props.readonly} blacklist={ListBlock.blacklist} whitelist={ListBlock.whitelist}
  ...
  private readonly shiftLevelBy = (component: EditText, value: number) => {
    ...
    this.setState({focus: component.text, selection: component.getSelection()});
  };
  ...
  private readonly insertNewContent = (component: EditText) => {
    ...
    this.setState({focus: next_text, selection: {start: 0, end: 0, forward: true}});
